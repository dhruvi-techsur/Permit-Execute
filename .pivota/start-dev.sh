...
# (none — compose entry requires no pre-exec setup)
...
# Compose entry: EXEC_CMD is `docker compose up --build` — single foreground
# process; the single-process retry loop applies (multi-process-template.md §6).
