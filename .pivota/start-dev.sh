...
# (none — compose entry requires no system-level pre-install setup)
...
# Compose has no lock file and no install command — docker compose up --build
# handles all image pulls and rebuilds internally. Skip the install block.
